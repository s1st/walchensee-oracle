...
_UI: dict[str, dict[str, str]] = {
    "de": {
        ...
        "stats_forecast_header": "Vorhersage-Qualität (alle Saisons, Apr–Okt)",
        "stats_panel_live_header": "Live-Modelle (ganze Historie)",
        "stats_panel_live_note": "Was heute im Einsatz ist, bewertet über die gesamte Saison-Historie (Apr–Okt, ~10 Jahre): die Regel und das destillierte logistische Modell. Beide auf demselben Tage-Set.",
        "stats_panel_holdout_header": "Fairer Vergleich (Holdout ab 2023)",
        "stats_panel_holdout_note": "Direkter Drei-Wege-Vergleich auf identischem Tage-Set: nur die ungesehenen Testjahre ab 2023, für alle drei Modelle out-of-sample. Regel plus die im Bundle gespeicherten, bis 2022 trainierten Logistik- und HGB-Modelle — so wird HGB nicht auf seinen eigenen Trainingsjahren bewertet.",
        "stats_name_rule": "Regel-basiert",
        "stats_name_ml": "ML — Logistisch",
        "stats_name_hgb": "ML — HGB (Blackbox)",
        "stats_sample_size": "Bewertete Tage",
        "stats_skill": "Prognoseskill",
        "stats_skill_note": "Peirce-Score (Sensitivität + Spezifität − 1). 0 = nicht besser als Raten, 1 = perfekt. Positiv = nützlich.",
        "stats_sensitivity_headline": "Windtage erkannt",
        "stats_sensitivity_headline_note": "Anteil echter Wind-Tage (≥ 1 h ≥ 8 kt), an denen das Orakel nicht FLAUTE gesagt hat.",
        "stats_specificity_headline": "Fehlfahrten vermieden",
        "stats_specificity_headline_note": "Anteil echter Flaute-Tage, die korrekt als FLAUTE vorhergesagt wurden.",
        "stats_accuracy": "3-Klassen-Trefferquote",
        ...
        "stats_hgb_note": "Vorhersage = HistGradientBoosting. Trainiert auf den Jahren bis 2022, deshalb hier nur auf den ungesehenen Testjahren ab 2023 bewertet (anderes, kleineres Tages-Set als oben). Das stärkste Offline-Modell, aber Blackbox und ohne Prod-Deployment. Nur Vergleich.",
        ...
        "about_glossary_header": "Glossar",
        "about_glossary_intro": "Kurze Erklärungen zu den Fachbegriffen, die in den Regeln und in der Tagesbegründung auftauchen.",
        "glossary_hint": "Fachbegriffe erklärt",
    ...
    "en": {
        ...
        "stats_forecast_header": "Forecast quality (all seasons, Apr–Oct)",
        "stats_panel_live_header": "Live models (full history)",
        "stats_panel_live_note": "What's actually deployed, scored over the whole seasonal history (Apr–Oct, ~10 years): the rule and the distilled logistic model. Both on the same day set.",
        "stats_panel_holdout_header": "Fair comparison (held out, 2023+)",
        "stats_panel_holdout_note": "A like-for-like three-way comparison on one shared day set: only the unseen test years from 2023 on, out-of-sample for all three. The rule plus the bundle's logistic and HGB models trained up to 2022 — so HGB isn't graded on its own training years.",
        "stats_name_rule": "Rule-based",
        "stats_name_ml": "ML — logistic",
        "stats_name_hgb": "ML — HGB (black box)",
        "stats_sample_size": "Days scored",
        "stats_skill": "Forecast skill",
        "stats_skill_note": "Peirce score (sensitivity + specificity − 1). 0 = no better than guessing, 1 = perfect. Positive = useful.",
        "stats_sensitivity_headline": "Wind days caught",
        "stats_sensitivity_headline_note": "Share of real wind days (≥ 1 h ≥ 8 kt) where the oracle did not say NO GO.",
        "stats_specificity_headline": "Wasted drives avoided",
        "stats_specificity_headline_note": "Share of real no-wind days that were correctly forecast as NO GO.",
        "stats_accuracy": "3-class accuracy",
        ...
        "stats_hgb_note": "Forecast = HistGradientBoosting. Trained on the years up to 2022, so scored here only on the unseen test years from 2023 on (a different, smaller day set than above). The strongest offline model, but a black box with no prod deployment. Comparison only.",
        ...
        "about_glossary_header": "Glossary",
        "about_glossary_intro": "Short explanations of the technical terms that show up in the rules and the daily summary.",
        "glossary_hint": "Technical terms explained",
        ...
        "index_glossary_link": "Glossary",
    },
}


# Glossary of the meteorological + forecasting jargon that surfaces bare in the
# rule reasons, the verdict summary line, and the stats page. Rendered on the
# About ("Erklärung") page under an anchored <dl id="glossar">; the hint links
# next to the summary and on the stats page deep-link here. Ordered the way a
# reader meets the terms: basics → pressure → sun/moisture → stability → aloft →
# stats. `id` becomes the per-term anchor (glos-<id>).
_GLOSSARY: dict[str, list[dict[str, str]]] = {
    "de": [
        {"id": "thermik", "term": "Thermik (thermischer Wind)",
         "def": "Lokaler Wind, den die Sonne antreibt: Erwärmte Luft an den Hängen steigt auf, kühlere Luft strömt nach. Am Walchensee ergibt das den verlässlichen Nordwind, auf den Surfer und Kiter warten."},
        {"id": "hangheizung", "term": "Hangheizung",
         "def": "Das Aufheizen der besonnten Berghänge durch die Sonne. Sie ist der Motor der Thermik — ohne Sonne auf den Hängen keine aufsteigende Luft."},
        {"id": "zuendung", "term": "Zündung (Thermik-Zündung)",
         "def": "Der Moment, in dem der thermische Wind einsetzt und kräftig genug zum Surfen wird (am Walchensee ab etwa 8 Knoten Mittelwind)."},
        {"id": "knoten", "term": "Knoten (kt)",
         "def": "Einheit für die Windgeschwindigkeit. 1 Knoten ≈ 1,85 km/h. Faustwerte: 8 kt ≈ 15 km/h, 11 kt ≈ 20 km/h."},
        {"id": "boe", "term": "Böe",
         "def": "Kurzzeitige Windspitze, deutlich stärker als der Mittelwind. Wichtig für Materialwahl und Gefühl auf dem Wasser."},
        {"id": "hpa", "term": "hPa (Hektopascal)",
         "def": "Einheit für den Luftdruck. Die Vorhersage schaut weniger auf den absoluten Wert als auf die Differenz zwischen zwei Orten."},
        {"id": "druckgradient", "term": "Druckgradient · Δ (Delta)",
         "def": "Die Luftdruck-Differenz zwischen zwei Orten (z. B. München − Innsbruck). Luft strömt vom hohen zum tiefen Druck — ein passender Gradient schiebt den Nordwind über die Alpen zum See. „Δ“ ist das Zeichen für diese Differenz."},
        {"id": "foehn", "term": "Föhn",
         "def": "Warmer, kräftiger Südwind, der über den Alpenhauptkamm absinkt. Er bringt zwar Wind, verdrängt und zerstört aber die lokale Thermik. Erkennbar am Druckunterschied Bozen − Innsbruck."},
        {"id": "inversion", "term": "Inversion",
         "def": "Eine Luftschicht, in der es nach oben wärmer statt kälter wird. Sie wirkt wie ein Deckel und bremst aufsteigende warme Luft — schlecht für die Thermik. Eine klare, kalte Nacht baut die nötige Abkühlung am Boden auf."},
        {"id": "taupunkt", "term": "Taupunkt · Taupunkt-Abstand",
         "def": "Der Taupunkt ist die Temperatur, bei der die Luft gesättigt wäre. Der Abstand zur tatsächlichen Temperatur (T − Taupunkt) zeigt, wie trocken die Luft ist. Kleiner Abstand = feuchte Luft: Sonnenenergie verdunstet Wasser, statt die Luft zu erwärmen."},
        {"id": "einstrahlung", "term": "Sonneneinstrahlung (W/m²)",
         "def": "Wie viel Sonnenleistung pro Quadratmeter auf den Boden trifft (Watt pro Quadratmeter). Mehr Einstrahlung = stärkere Hangheizung."},
        {"id": "bodenfeuchte", "term": "Bodenfeuchte (m³/m³)",
         "def": "Wassergehalt des Bodens (Kubikmeter Wasser pro Kubikmeter Boden). Nasser Boden nach Regen heizt schlechter auf, weil die Sonnenenergie ins Verdunsten geht statt in die Erwärmung."},
        {"id": "grenzschicht", "term": "Grenzschicht (Grenzschichthöhe, BLH)",
         "def": "Die unterste, gut durchmischte Luftschicht über dem Boden. Je höher sie reicht, desto mehr Raum hat die Thermik nach oben. Eine flache Grenzschicht „deckelt“ den Wind. (Englisch boundary layer height, BLH.)"},
        {"id": "lifted-index", "term": "Lifted Index (LI)",
         "def": "Ein Maß für die Stabilität der Atmosphäre. Hohe positive Werte = sehr stabil, die Thermik wird gedeckelt. Stark negative Werte = labil, Gewittergefahr. Der ideale Bereich liegt dazwischen."},
        {"id": "cape", "term": "CAPE",
         "def": "„Convective Available Potential Energy“ — die Energie, die für aufsteigende Luft und Gewitter zur Verfügung steht. Wie der Lifted Index ein Stabilitätsmaß: viel CAPE bedeutet kräftige Aufwinde, aber auch Gewitterrisiko."},
        {"id": "hoehenwind", "term": "Höhenwind · 850 / 700 hPa",
         "def": "Statt in Metern werden Höhen oft als Druckflächen angegeben: 850 hPa entspricht etwa 1 500 m, 700 hPa etwa 3 000 m. Der Wind in diesen Höhen entscheidet, ob die Großwetterlage die bodennahe Thermik stört."},
        {"id": "querstroemung", "term": "Querströmung",
         "def": "Wind in der Höhe, der quer zur Tal-Achse weht. Zu viel davon entkoppelt das Tal von der Höhe und stört die saubere Thermik."},
        {"id": "synoptik", "term": "Synoptik (synoptischer Wind)",
         "def": "Der Wind der großräumigen Wetterlage (Hochs und Tiefs), im Gegensatz zum lokalen Thermikwind. Ist er zu stark, überdeckt er die zarte Thermik."},
        {"id": "peirce", "term": "Peirce-Score",
         "def": "Ein Treffer-Maß für die Vorhersage (Sensitivität + Spezifität − 1). 0 = nicht besser als Raten, 1 = perfekt. Mehr dazu auf der Statistik-Seite."},
    ],
    "en": [
        {"id": "thermik", "term": "Thermal wind",
         "def": "A local wind driven by the sun: air warmed on the slopes rises and pulls cooler air in behind it. At Walchensee this produces the reliable north wind that surfers and kiters wait for."},
        {"id": "hangheizung", "term": "Slope heating",
         "def": "The sun warming the sun-facing mountain slopes. It's the engine of the thermal — no sun on the slopes, no rising air."},
        {"id": "zuendung", "term": "Ignition (thermal ignition)",
         "def": "The moment the thermal wind kicks in and grows strong enough to ride (at Walchensee from about 8 knots mean wind)."},
        {"id": "knoten", "term": "Knots (kt)",
         "def": "The unit for wind speed. 1 knot ≈ 1.85 km/h. Rules of thumb: 8 kt ≈ 15 km/h, 11 kt ≈ 20 km/h."},
        {"id": "boe", "term": "Gust",
         "def": "A short-lived wind peak, clearly stronger than the mean wind. Matters for kit choice and how the water feels."},
        {"id": "hpa", "term": "hPa (hectopascal)",
         "def": "The unit for air pressure. The forecast cares less about the absolute value than about the difference between two places."},
        {"id": "druckgradient", "term": "Pressure gradient · Δ (delta)",
         "def": "The air-pressure difference between two places (e.g. Munich − Innsbruck). Air flows from high to low pressure — the right gradient pushes the north wind over the Alps to the lake. „Δ“ is the symbol for that difference."},
        {"id": "foehn", "term": "Föhn",
         "def": "A warm, strong south wind that sinks across the main Alpine ridge. It brings wind but displaces and destroys the local thermal. Detected from the Bolzano − Innsbruck pressure difference."},
        {"id": "inversion", "term": "Inversion",
         "def": "A layer where the air gets warmer with height instead of colder. It acts like a lid and stops warm air from rising — bad for the thermal. A clear, cold night builds the cooling needed near the ground."},
        {"id": "taupunkt", "term": "Dew point · dew-point spread",
         "def": "The dew point is the temperature at which the air would be saturated. Its gap to the actual temperature (T − dew point) shows how dry the air is. A small gap = humid air: solar energy evaporates water instead of heating the air."},
        {"id": "einstrahlung", "term": "Solar radiation (W/m²)",
         "def": "How much solar power reaches the ground per square metre (watts per square metre). More radiation = stronger slope heating."},
        {"id": "bodenfeuchte", "term": "Soil moisture (m³/m³)",
         "def": "The water content of the soil (cubic metres of water per cubic metre of soil). Wet ground after rain heats up poorly, because solar energy goes into evaporation instead of warming."},
        {"id": "grenzschicht", "term": "Boundary layer (boundary-layer height, BLH)",
         "def": "The lowest, well-mixed layer of air above the ground. The higher it reaches, the more room the thermal has to grow. A shallow boundary layer „caps“ the wind."},
        {"id": "lifted-index", "term": "Lifted index (LI)",
         "def": "A measure of how stable the atmosphere is. High positive values = very stable, the thermal is capped. Strongly negative values = unstable, thunderstorm risk. The sweet spot is in between."},
        {"id": "cape", "term": "CAPE",
         "def": "“Convective available potential energy” — the energy available for rising air and thunderstorms. Like the lifted index, a stability measure: lots of CAPE means strong updraughts but also thunderstorm risk."},
        {"id": "hoehenwind", "term": "Upper-level wind · 850 / 700 hPa",
         "def": "Heights are often given as pressure levels rather than metres: 850 hPa is roughly 1,500 m, 700 hPa roughly 3,000 m. The wind at these heights decides whether the large-scale weather disturbs the surface thermal."},
        {"id": "querstroemung", "term": "Crossflow",
         "def": "Wind aloft that blows across the valley axis. Too much of it decouples the valley from the air above and disturbs a clean thermal."},
        {"id": "synoptik", "term": "Synoptic flow",
         "def": "The wind of the large-scale weather pattern (highs and lows), as opposed to the local thermal wind. When it's too strong it overrides the delicate thermal."},
        {"id": "peirce", "term": "Peirce score",
         "def": "A skill measure for the forecast (sensitivity + specificity − 1). 0 = no better than guessing, 1 = perfect. More on the Stats page."},
    ],
}
