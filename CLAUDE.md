...
`storm_classifier` is a **pure-Python logistic** over 9 afternoon (12–18 local) convective features — CAPE, LI, CIN, precip, deep shear, low-cloud + 3 interactions — computed by `pillars/meteo.py` and stored as `afternoon_*` keys on `MeteoSnapshot`. On 5 seasons (2021–2025) of **stored buoy gust-front ground truth** (`data/runs/<iso>.json` `ground_truth.machine.samples`: gust ≥ 22 kt ∧ MSL pressure jump ≥ 2 hPa) it scores leave-one-year-out POD 82 % / FAR 84 % / Peirce 0.431 vs the old LI flag's 0.178 — ~2× recall at a lower false-alarm ratio. When the afternoon features are absent (archive host / pre-2021 / legacy records) it **falls back to the LI ≤ −2 rule**.

**Operating point is a product knob, not a model artifact:** the live threshold is `config.STORM_ADVISORY_THRESHOLD` (currently **0.6**) — deliberately *not* `storm_coeffs.THRESHOLD` (the export's recall-favouring ~0.398, which a retrain overwrites). The export's POD-82 point flagged ~half of June days (alarm fatigue, ~16/30 vs 3 real storms); 0.6 ~halves that to ~LI density, trading recall for a calmer advisory. The border is computed **live** by `storm_suspected`, so changing the knob takes effect on the next deploy with **no rescore**. Nudge it down (~0.5) if users report missed storms.

**Calibration scores storm days — it does NOT quarantine them (and must not start).** The old LI≤−2 quarantine (excluding forecast-storm days from the confusion matrix) was lifted with the decouple. Storm days are now scored like any other against the **`thermal`** label (`stats_cache.py` → `compile_report(label="thermal")`), which de-contaminates gust-front wind: a front's brief high wind is relabelled NO_GO (not a thermal session), so it can't fake a "session" the way the raw `peak` label does — measured on observed-storm days, `peak` calls 96 % GO while `thermal` correctly NO_GO's ~half (102/210). **Do not re-introduce a storm quarantine** — not even one keyed on *observed* storms (`calibration.observed_storm`) rather than the forecast. It would discard exactly the storm-day-with-a-real-thermal cases the decouple exists to measure (the decoupled rules catch ~69 % of them); the `thermal` label already handles the contamination per-day without dropping data. `storm_suspected` in `compile_report` is now only a tally for the dashboard advisory, not an exclusion.
...
`src/oracle/dashboard/main.py` (FastAPI + Jinja2, single `index.html`) reads from the same `RunStore` the CLI writes. In-process caches: 60 s per-day file, 5 min for the live-Urfeld panel, 6 h for the forecast-stats payload, 12 h for the visitor counts — keep them in mind when changing endpoints, they mask staleness.

**Both expensive panels are precomputed off the request path, never walked inline.** The forecast-stats payload (the replay calibration walk) is written by `oracle stats-update` → `runs/_stats_cache.json`; the visitor counts (a 30-day Cloud Logging walk, ~tens of seconds) by `oracle views-update` → `runs/_views_cache.json` (`src/oracle/views_cache.py`, mirroring `stats_cache.py`). The dashboard serves both with a single cheap GCS read. `views-update` is called **best-effort at the end of the scheduled multi-day `forecast`** (`_refresh_views_cache_best_effort`, gated on a cloud env via `CLOUD_RUN_JOB`/`K_SERVICE`/`LOG_PROJECT`, swallows all errors) so the counts refresh daily on the existing forecast job with no separate Cloud Run job. If the blob is missing, the dashboard falls back to running the walk itself in the background (non-blocking; the stats template polls every 8 s via `views_pending`) — so it degrades gracefully pre-migration. The walk targets the dashboard *service* (`walchi-oracle-dash`) by default even from the job, because Cloud Run Jobs don't set `K_SERVICE`. **For the job to write the views cache, `walchi-oracle-job@` needs `roles/logging.viewer`** (the dashboard SA already has it; granted 2026-06-29); `google-cloud-logging` was moved to base deps (was `[dashboard]`) so the job image ships it.

**Two refresh cadences for the forecast-stats panel, kept separate.** (1) *Rule changes* → event-driven: rescore both namespaces + `stats-update` at deploy (the runbook below). (2) *New live-season days* → the `oracle-backfill` job (21:00 CET) folds them in **weekly**, Sundays only, via `_refresh_stats_panel_weekly_best_effort` (best-effort, cloud-env-gated, in-season-gated on `config.ACTIVE_SEASON_MONTHS`). It re-replays a trailing 14-day window (idempotent overwrite; overlap survives a missed week), `rescore --replayed --since` those days, then `stats-update`. `stats-update` alone is pointless on a timer — it re-walks the *same* corpus to byte-identical output; the replay+rescore is what adds the new days. This runs in-cloud because the job image bakes the ML bundle (`COPY data/ml/replay_full.pkl`) + `[hgb]` sklearn, so it can build the HGB holdout panel. No dashboard bounce needed: the 6 h stats TTL self-clears (the `STATS_CACHE_BUST` bump is only to force *instant* pickup after a rule change). Off-season nothing fires (winter shutdown). **Do not reintroduce server-side scraping or republishing of the windinfo.eu Wind-Wetter-Chat** — third-party user posts have DSGVO + § 87b UrhG (Datenbankschutz) implications, and the chat pillar was deliberately removed for that reason. (The footer's friendly link to that chat was also removed 2026-06-24 — no longer needed.) The footer now credits the Addicted-Sports partnership explicitly ("used with kind permission of and in partnership with"), which doubles as a record of the data-use consent.
...
- **The build updates the dashboard *service* but NOT the *jobs*.** `cloudbuild.yaml`'s deploy step only runs `gcloud run services update` (for `_DEPLOY_SERVICE`); the job trigger has `_DEPLOY_SERVICE=''`, so it builds+pushes `oracle-job:latest` but never re-points the jobs. Cloud Run jobs pin the digest at update time, so **after a build that changes rule logic you must re-pin both jobs** or the next scheduled forecast runs the old code:
  ...
  ```
- **After any change to *verdict* logic, rescore BOTH namespaces** — `runs/` (live records, drives day-detail) **and** `runs/replay/` (the ~3,300-day corpus the **stats panel** scores). Rescoring only `runs/` leaves the stats panel advertising the old rules; rescoring only updates `overall_resimulated`/`verdicts_resimulated` (never `overall`/`verdicts`), so it's additive + idempotent — no backup strictly needed. Then regenerate the cache + bounce:
  ```
  RUNS_BUCKET=walchi-oracle-prod-runs oracle rescore --since <PROJECT_FIRST_DAY>   # live records (day-detail)
  RUNS_BUCKET=walchi-oracle-prod-runs oracle rescore --replayed                    # replay corpus (stats panel) — ~10-15 min
  RUNS_BUCKET=walchi-oracle-prod-runs oracle stats-update                          # regenerate _stats_cache.json
  gcloud run services update walchi-oracle-dash --region europe-west1 --update-env-vars STATS_CACHE_BUST=<sha>  # drop the 1h in-process cache
  ```
  (Learned 2026-06-24: the LI-decouple deploy skipped the rescore, so the resimulated day-detail kept showing the old HARD veto; and `runs/_stats_cache.json` in the bucket once crashed the day-walk — now handled. A storm-*advisory* threshold change needs none of this — the border is computed live.)
